...
class ArcballControl {
  ...
  constructor(private canvas: HTMLCanvasElement, private updateCallback: UpdateCallback = () => undefined) {
  ...
  public update(deltaTime: number, targetFrameDuration = 16) {
    ...
    const rvIntensity = 0.5 * timeScale
    this._rotationVelocity += (rv - this._rotationVelocity) * rvIntensity
...
class InfiniteGridMenu {
  ...
  private run = (time = 0) => {
    if (!this.isRunning) return
    this._deltaTime = Math.min(32, time - this._time || this.TARGET_FRAME_DURATION)
    this._time = time
...
interface InfiniteMenuProps {
  ...
  enabled?: boolean
  hideOverlay?: boolean
  onActiveChange?: (item: MenuItem | null) => void
  onMovementChange?: (isMoving: boolean) => void
}

const InfiniteMenu: FC<InfiniteMenuProps> = ({ items, scale = 1, enabled = true, hideOverlay = false, onActiveChange, onMovementChange }) => {
  ...
  useEffect(() => {
    ...
    if (!sketchRef.current) {
      sketchRef.current = new InfiniteGridMenu(
        canvas,
        items,
        index => setActiveItem(items[index % items.length] ?? null),
        setIsMoving,
        scale,
      )
    }
  ...
  return (
    <div className="relative h-full w-full overflow-hidden rounded-full aspect-square">
      <canvas
        ref={canvasRef}
        className={`relative h-full w-full cursor-grab outline-none active:cursor-grabbing ${enabled ? 'opacity-100' : 'opacity-0'} transition-opacity duration-300`}
      />

      {!hideOverlay && activeItem && (
        <>
          <div
            className={`pointer-events-none absolute left-8 top-8 max-w-[18rem] transition-all ease-out ${
              isMoving ? 'translate-y-2 opacity-0 duration-100' : 'translate-y-0 opacity-100 duration-500'
            }`}
          >
            <p className="text-xs font-semibold uppercase tracking-[0.26em] text-[var(--text-soft)]">Capacidad activa</p>
            <h3 className="mt-3 text-3xl font-semibold tracking-[-0.03em] text-[var(--text)]">{activeItem.title}</h3>
          </div>

          <div
            className={`pointer-events-none absolute bottom-8 right-8 max-w-[18rem] text-right transition-all ease-out ${
              isMoving ? 'translate-y-2 opacity-0 duration-100' : 'translate-y-0 opacity-100 duration-500'
            }`}
          >
            <p className="text-sm leading-7 text-[var(--text-soft)]">{activeItem.description}</p>
          </div>
        </>
      )}
    </div>
